...
<div class="test-form">
    ...
    <div class="body">
        <div class="right">
            <form method="POST" action="{{ route('essay.update', ['id' => $baiKiemTra->msbkt, 'malop' => $malop]) }}"
...
<script>
    document.getElementById('num-questions').addEventListener('input', function() {
        ...
        if (pointsPerQuestion <= 0 || pointsPerQuestion > 10) {
            alert('Tổng điểm không hợp lệ. Mỗi câu phải có điểm hợp lệ.');
            return;
        }
        ...
        for (let i = 0; i < numQuestions; i++) {
            totalPoints += parseFloat(pointsPerQuestion);
        ...
        if (totalPoints !== 10) {
            alert(`Tổng điểm hiện tại là ${totalPoints}. Tổng điểm phải là 10.`);
